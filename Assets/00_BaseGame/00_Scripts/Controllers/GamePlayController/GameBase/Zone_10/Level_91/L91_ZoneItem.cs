...
public class L91_ZoneItem : MonoBehaviour
{
    public BoxCollider2D boxCollider2d;
    public List<Transform> lsPoints = new List<Transform>();

    // Lưu trữ các item đang gắn vào từng điểm
    private List<L91_Item> assignedItems;

    private void Start()
    {
        assignedItems = new List<L91_Item>(new L91_Item[lsPoints.Count]);
    }
    ...
    public Vector3 AssignItemToNearestAvailable(L91_Item item, Vector3 position)
    {
        ...
        for (int i = 0; i < lsPoints.Count; i++)
        {
            ...
            float distance = Vector3.Distance(position, lsPoints[i].position);
        ...
        if (nearestIndex >= 0 && minDistance < 0.3f)
        {
            ...
            return lsPoints[nearestIndex].position; // Trả về vị trí của điểm được gán
        }
        ...
        if (firstEmpty >= 0)
        {
            ...
            return lsPoints[firstEmpty].position; // Trả về vị trí của điểm được gán
        }
    ...
    {
        if (index >= 0 && index < lsPoints.Count)
            return lsPoints[index].position;
        return Vector3.zero;
    }
}